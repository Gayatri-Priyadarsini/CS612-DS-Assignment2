...
// Always stop a electionTimer before reusing it.
...
// ask vote from other replicas
func (rf *Raft) askForVote(server int, args RequestVoteArgs, replyCh chan<- RequestVoteReply) {
...
func (rf *Raft) ticker() {
	...
	for i := 0; i < len(rf.peers); i++ {
		if i != me {
			go rf.askForVote(i, args, replyCh)
		}
	}
	...
	for voteCount < threshold {
		...
		case reply := <-replyCh:
			if reply.Err != OK {
				go rf.askForVote(reply.Server, args, replyCh)
